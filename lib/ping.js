    ...
    s.on('end', function () {
        try {
            ...
            callback("", {
                ...
                version: 0,
                latency: parseInt(100),
                average: response.average.month
            });
        } catch (err) {
            console.log(err)
            callback(err, null);
...
// Ping the Legacy Minecraft Server API
function pingLegacyMinecraftAPI(host, port, timeout, callback) {
    var startTime = util.getCurrentTimeMs();

    getJSON("https://servers.api.legacyminecraft.com/api/v1/getServer?uuid=" + host + "&icons=true", function (error, res) {
        if (error) {
            console.log("Ping Error Code: 1 -- " + error)
            callback(error, null);
        } else {
            if(res.found) {
                //Get online count
                ...
                for(var i = 0; i < res.players.length; i++) {
                    if(parseInt(res.players[i].secondsSinceLastMove) <= 300) {
                        onlinePLayers = onlinePLayers + 1;
                    }
                }
                let serverIcon = null;
                if(res.hasOwnProperty("serverIcon")) {
                    serverIcon= "data:image/png;base64," + res.serverIcon;
                }
...
exports.ping = function (host, port, type, timeout, callback, version) {
    ...
    } else if (type === "LEGACYMCAPI") {
        pingLegacyMinecraftAPI(host, port || 19132, timeout, callback);
    }else {
